...
| [RP Setup Target Stage Turn Entry Contract](./rp-setup-target-stage-turn-entry-contract.md) | Carry canonical `target_stage` through setup turn ingress and keep `target_step` only as a compatibility mirror | Active |
| [RP Setup Legacy Step Mirror Convergence](./rp-setup-legacy-step-mirror-convergence.md) | Keep `current_step` synchronized as the compatibility mirror of canonical stage progression and centralize stage-to-step bridge logic | Active |
...
| [RP Runtime Identity Persistence And Propagation](./rp-runtime-identity-persistence-propagation.md) | Boot-bar contract for persistent `BranchHead` / `StoryTurn` records and deterministic runtime identity propagation across memory-owned paths | Active |
| [RP Runtime Profile Snapshot Minimal Compiler](./rp-runtime-profile-snapshot-minimal-compiler.md) | Boot-bar contract for deterministic, immutable `RuntimeProfileSnapshot` compilation and turn-start pinning from registry/config inputs | Active |
| [RP Runtime Workspace Persistent Turn Material Store](./rp-runtime-workspace-persistent-turn-material-store.md) | Boot-bar contract for durable Runtime Workspace turn-material persistence keyed by full runtime identity without promoting Workspace into story truth | Active |
| [RP Persistent Memory Event Record Foundation](./rp-persistent-memory-event-record-foundation.md) | Boot-bar contract for durable identity-scoped memory event records used for trace and invalidation without becoming a truth store | Active |
| [RP Branch Visibility Resolver And Lineage](./rp-branch-visibility-resolver-lineage.md) | Boot-bar contract for lineage-aware branch visibility across Core, Projection, Runtime Workspace, Recall, and Retrieval runtime reads | Active |
| [RP Core Projection And Read Manifest Hardening](./rp-core-projection-read-manifest-hardening.md) | Boot-bar contract for strict fact/view separation, runtime-owned projection refresh identity requirements, and deterministic packet-visible read manifests | Active |
| [RP Retrieval Card Usage And Promotion Boot Contract](./rp-retrieval-card-usage-promotion-boot-contract.md) | Boot-bar contract for search-to-card materialization, explicit writer retrieval usage, and governed post-write source-ref promotion inputs | Active |
| [RP Worker Memory Tool And Permission Boot Contract](./rp-worker-memory-tool-permission-boot-contract.md) | Boot-bar contract for shared internal worker-facing memory services, snapshot-derived permission guards, and governed proposal/refresh metadata | Active |
| [RP Shared Core Mutation Kernel And Direct Edit](./rp-shared-core-mutation-kernel-direct-edit.md) | Boot-bar contract for one governed authoritative Core mutation kernel shared by direct edit, worker apply, and brainstorm summary apply paths | Active |
| [RP Recall Branch-Aware Lifecycle](./rp-recall-branch-aware-lifecycle.md) | Full-foundation contract for Recall lifecycle metadata, branch-aware historical materialization, and governed supersede/invalidate/recompute semantics | Active |
| [RP Archival Evolution Reindex Governance](./rp-archival-evolution-reindex-governance.md) | Full-foundation contract for versioned Archival Story Evolution edits, visibility scopes, reindex linkage, and source-version provenance | Active |
| [RP Memory Event Debug Eval Read Surfaces](./rp-memory-event-debug-eval-read-surfaces.md) | Full-foundation contract for exact-identity memory trace/debug/eval reads over persisted events, materials, manifests, and proposal receipts | Active |
| [RP Registry Profile Snapshot Full Management](./rp-registry-profile-snapshot-full-management.md) | Full-foundation contract for persistent domain/block/worker/profile management and immutable snapshot compilation beyond bootstrap defaults | Active |
| [RP User-Visible Memory Inspection Edit Backend Contracts](./rp-user-visible-memory-inspection-edit-backend-contracts.md) | Full-foundation contract for branch-aware inspection plus governed Core/Recall/Archival user correction backends | Active |
| [RP User-Visible Memory Canonical JSON Governance](./rp-user-visible-memory-canonical-json-governance.md) | Canonical block/entry envelope contract that binds user-visible memory UI payloads to governed backend edit/review/evolution semantics | Active |
...
- [ ] If the change touches setup turn ingress, `target_stage`, request/graph-shell target selection, or the `current_stage` vs `current_step` compatibility boundary, read [RP Setup Target Stage Turn Entry Contract](./rp-setup-target-stage-turn-entry-contract.md).
- [ ] If the change touches canonical-stage to legacy-step mirror synchronization, `current_step` compatibility semantics, or stage-to-step bridge helper reuse, read [RP Setup Legacy Step Mirror Convergence](./rp-setup-legacy-step-mirror-convergence.md).
...
- [ ] If the change adds or modifies persistent `BranchHead` / `StoryTurn` records, runtime identity allocation/resolution, or full-identity propagation on runtime-owned memory paths, read [RP Runtime Identity Persistence And Propagation](./rp-runtime-identity-persistence-propagation.md).
- [ ] If the change adds or modifies `RuntimeProfileSnapshot` compile/publish/pin behavior, mode/domain/worker activation compilation, retrieval/permission/context policy pinning, or snapshot activation semantics, read [RP Runtime Profile Snapshot Minimal Compiler](./rp-runtime-profile-snapshot-minimal-compiler.md).
- [ ] If the change adds or modifies durable Runtime Workspace turn-material records, repository-backed identity-scoped material queries, or persistent lifecycle storage, read [RP Runtime Workspace Persistent Turn Material Store](./rp-runtime-workspace-persistent-turn-material-store.md).
- [ ] If the change adds or modifies durable memory event records, repository-backed exact-identity event queries, or persistent dirty-target readback, read [RP Persistent Memory Event Record Foundation](./rp-persistent-memory-event-record-foundation.md).
- [ ] If the change adds or modifies branch lineage visibility, rollback visibility filtering, branch-scoped Runtime Workspace/Recall reads, or active-branch retrieval visibility, read [RP Branch Visibility Resolver And Lineage](./rp-branch-visibility-resolver-lineage.md).
- [ ] If the change adds or modifies strict Core fact vs derived Projection/View boundaries, runtime-owned projection refresh identity requirements, or deterministic packet-visible read manifests, read [RP Core Projection And Read Manifest Hardening](./rp-core-projection-read-manifest-hardening.md).
- [ ] If the change adds or modifies retrieval-card materialization, card expansion, writer retrieval usage records, or governed post-write promotion source refs, read [RP Retrieval Card Usage And Promotion Boot Contract](./rp-retrieval-card-usage-promotion-boot-contract.md).
- [ ] If the change adds or modifies shared internal worker-facing memory contracts, snapshot-derived worker permission guards, or worker proposal/refresh governance metadata, read [RP Worker Memory Tool And Permission Boot Contract](./rp-worker-memory-tool-permission-boot-contract.md).
- [ ] If the change adds or modifies authoritative Core direct edit routing, shared mutation envelopes, or alignment between user direct edit, worker apply, and brainstorm summary apply, read [RP Shared Core Mutation Kernel And Direct Edit](./rp-shared-core-mutation-kernel-direct-edit.md).
- [ ] If the change adds or modifies Recall lifecycle metadata, branch-aware historical visibility, or Recall supersede/invalidate/recompute behavior, read [RP Recall Branch-Aware Lifecycle](./rp-recall-branch-aware-lifecycle.md).
- [ ] If the change adds or modifies Archival Story Evolution edits, source/chunk versioning, visibility scopes, or reindex governance, read [RP Archival Evolution Reindex Governance](./rp-archival-evolution-reindex-governance.md).
- [ ] If the change adds or modifies memory trace/debug/eval readbacks over persisted events, materials, manifests, or proposal receipts, read [RP Memory Event Debug Eval Read Surfaces](./rp-memory-event-debug-eval-read-surfaces.md).
- [ ] If the change adds or modifies persistent domain/block/worker descriptors, mode profiles, or immutable runtime profile snapshot management beyond bootstrap defaults, read [RP Registry Profile Snapshot Full Management](./rp-registry-profile-snapshot-full-management.md).
- [ ] If the change adds or modifies branch-aware memory inspection queries, user-visible Core direct edit, Recall review, or Archival evolution backend contracts, read [RP User-Visible Memory Inspection Edit Backend Contracts](./rp-user-visible-memory-inspection-edit-backend-contracts.md).
- [ ] If the change adds or modifies user-visible memory block/entry payload shape, editable-field metadata, allowed actions, validation summaries, or UI/worker shared memory envelopes, read [RP User-Visible Memory Canonical JSON Governance](./rp-user-visible-memory-canonical-json-governance.md).
...
- [ ] Recall lifecycle keeps retrieval-core as the physical store, preserves branch/turn/source/lifecycle metadata, and never becomes current-state truth.
- [ ] Archival evolution creates a versioned source/chunk/index chain with explicit visibility scope instead of mutating active archival rows in place.
- [ ] Memory trace/debug/eval reads join persisted events, materials, manifests, and proposal receipts by exact identity without replaying truth from events.
- [ ] Registry/profile full management keeps bootstrap defaults usable while allowing add/disable/migrate/publish/activate flows without hardcoding mode behavior into core runtime services.
