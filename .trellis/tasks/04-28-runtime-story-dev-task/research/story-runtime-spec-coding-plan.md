...
- UI 层可简化为：
  - 用户显式采用某一版时，该版成为确定版本；
  - 用户直接点击 `续写` 时，当前页面正在看的那一版自动晋升为确定版本。
...
- `post_write_deferred`：本轮允许把完整 post-write 延后，但延后状态必须被显式记录，供下一轮 gating 和补调度使用。
- `settled`：该 turn 的 acceptance 条件满足，且该 turn 的必需 post-write 已完成、被跳过、或被显式标记为 `post_write_deferred`。
