---
name: trellis-assign-task
description: "Assign the current session to a Trellis task. Use when a new session needs to pick an existing task, keep the repo default, or create a new task and bind it immediately."
---
...
Use this skill at the start of a new session when you need to decide:

- keep following the repo default task
- switch this session to an existing task
- create a new task and bind it to this session

This skill must prefer **session-scoped assignment** and must not overwrite the repo default unless the user explicitly asks for that.
...
- next recommended step: usually continue the assigned task
