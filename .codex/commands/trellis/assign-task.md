...
Bind the current Codex session to an existing Trellis task, or create a new task and bind it immediately.

> Codex note: in this repo, the authoritative executable entrypoint remains the shared skill `trellis-assign-task` under `.agents/skills/trellis-assign-task/`. This file is a project-local command mirror for discoverability and consistency.
